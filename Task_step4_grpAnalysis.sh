...
arrA=(1 3 5 7)												# setA beh sub-brik for compList. Must be same length as compList
arrB=(1 3)													# setB
arrC=(1 3 5)
wsArr=ABC													# list of within-subject arrays used (arrA, arrB, etc), for building permutations (e.g. wsArr=ABC)

namA=(Mask FBO UBO CA)										# names of behaviors from arrA. Must be same length as arrA
namB=(CA Odor)
namC=(CA Mask FUBO)
...
covSnif=(`tail -n +2 ${stimDir}/Cov_list.txt | awk '{print $3}'`)
covSPA=(`tail -n +2 ${stimDir}/Cov_list.txt | awk '{print $4}'`)
covIUS=(`tail -n +2 ${stimDir}/Cov_list.txt | awk '{print $5}'`)
covRBQ=(`tail -n +2 ${stimDir}/Cov_list.txt | awk '{print $6}'`)
